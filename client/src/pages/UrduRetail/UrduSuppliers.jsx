...
const UrduSuppliers = () => {
  ...
  const printPaymentReceipt = (data) => {
    ...
    const html = `
      ...
      <title>سپلائر ادائیگی رسید</title>
      <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        <h2>${shopName}</h2>
        <div class="dash"></div>
        <p class="title">سپلائر ادائیگی رسید</p>
        <div class="dash"></div>
        <div class="info">
          <p><b>رسید نمبر:</b> ${data.receiptId}</p>
          <p><b>تاریخ:</b> ${data.date.toLocaleString('ur-PK')}</p>
          <p><b>درج کردہ از:</b> ${data.cashier}</p>
          <p><b>سپلائر:</b> ${data.supplierName}</p>
          ${data.supplierPhone ? `<p><b>فون:</b> ${data.supplierPhone}</p>` : ''}
        </div>
        <div class="dash"></div>
        ...
        <div class="dash"></div>
        <p class="thanks">ادائیگی کامیابی سے ریکارڈ ہو گئی۔</p>
        <div class="footer">
  ...
  return (
    <div className="suppliers-container urdu-rtl" style={{ direction: 'rtl', fontFamily: 'Noto Nastaliq Urdu, sans-serif' }}>
      ...
      <main className="suppliers-main">
        <header className="suppliers-header">
          <div className="suppliers-title-group">
            <h1>سپلائرز اور وینڈرز</h1>
            <p>اپنے ہول سیلرز کے رابطے اور بقایا جات کا انتظام کریں</p>
          </div>
          <div className="header-actions" style={{ display: 'flex', gap: '1rem', alignItems: 'center' }}>
            <div className="search-wrapper" style={{ position: 'relative', width: '300px' }}>
              <Search size={18} style={{ position: 'absolute', left: '12px', top: '50%', transform: 'translateY(-50%)', color: '#94a3b8' }} />
              <input type="text" placeholder="سپلائرز تلاش کریں..." className="auth-input" style={{ padding: '0.6rem 2.5rem 0.6rem 1rem', width: '100%', fontSize: '0.9rem', borderRadius: '10px' }} value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
            </div>
            {!isCashier && (
              <button className="btn-primary" onClick={openAddModal}><Plus size={18} /> نیا سپلائر شامل کریں</button>
            )}
          </div>
        </header>
              ...
              <div key={supplier._id} className="supplier-card" style={{ display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
                ...
                <div>
                  ...
                  <span style={{
                    ...
                    {(supplier.totalDue || 0) > 0 ? `بقایا: Rs. ${supplier.totalDue.toLocaleString()}` : 'کلیئر ✓'}
                  </span>
          ...
          <div style={{ background: 'white', borderRadius: '16px', width: '90%', maxWidth: '420px', boxShadow: '0 25px 50px -12px rgba(0,0,0,0.25)', overflow: 'hidden' }}>
            <form onSubmit={handlePaymentSubmit}>
              <div style={{ display: 'flex', justifyContent: 'space-between', padding: '1.2rem 1.5rem', borderBottom: '1px solid #e2e8f0', background: '#f8fafc' }}>
                <h2 style={{ fontSize: '1.1rem', fontWeight: '700' }}>سپلائر کو ادائیگی</h2>
              ...
              <div style={{ padding: '1.5rem', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                <div>
                  <label style={{ fontSize: '0.85rem', fontWeight: '700', color: '#64748b', display: 'block', marginBottom: '0.4rem' }}>ادا شدہ رقم (Rs.)</label>
                  <input type="number" required placeholder="سپلائر کو ادا کی جانے والی رقم..." value={paymentData.amount} onChange={(e) => setPaymentData({ ...paymentData, amount: e.target.value })} style={{ width: '100%', padding: '0.8rem', borderRadius: '8px', border: '1px solid #cbd5e1', fontSize: '1rem', outline: 'none' }} />
                </div>
                <div>
                ...
                <div>
                  <label style={{ fontSize: '0.85rem', fontWeight: '700', color: '#64748b', display: 'block', marginBottom: '0.4rem' }}>نوٹ (اختیاری)</label>
                  <textarea placeholder="نوٹ شامل کریں..." value={paymentData.description} onChange={(e) => setPaymentData({ ...paymentData, description: e.target.value })} style={{ width: '100%', padding: '0.8rem', borderRadius: '8px', border: '1px solid #cbd5e1', fontSize: '0.9rem', outline: 'none', height: '75px', resize: 'none' }} />
                </div>
                <button type="submit" style={{ background: '#10b981', color: 'white', border: 'none', padding: '1rem', borderRadius: '10px', fontSize: '1rem', fontWeight: '700', cursor: 'pointer' }}>
                  ادائیگی کی تصدیق اور ریکارڈ کریں
                </button>
              </div>
            </form>
          </div>
        ...
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(15,23,42,0.6)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
          <div style={{ background: 'white', borderRadius: '16px', width: '95%', maxWidth: '1100px', maxHeight: '90vh', boxShadow: '0 25px 50px -12px rgba(0,0,0,0.25)', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', padding: '1.5rem 2rem', borderBottom: '1px solid #e2e8f0', background: '#f8fafc' }}>
              <h2 style={{ fontSize: '1.25rem', fontWeight: '800', color: '#0f172a' }}>📒 سپلائر لیجر ہسٹری</h2>
            ...
            <div style={{ overflowY: 'auto', flex: 1 }}>
              {ledgerLoading ? (
                <div style={{ padding: '4rem', textAlign: 'center', color: '#94a3b8' }}>لیجر لوڈ ہو رہا ہے...</div>
              ) : (
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.92rem' }}>
                  <thead style={{ position: 'sticky', top: 0, background: '#f8fafc', zIndex: 10 }}>
                    <tr style={{ borderBottom: '2px solid #e2e8f0', textAlign: 'left' }}>
                      ...
                      <th style={{ padding: '1rem 1.5rem', color: '#475569', fontWeight: '700', textAlign: 'right' }}>ڈیبٹ (خریداری)</th>
                      <th style={{ padding: '1rem 1.5rem', color: '#475569', fontWeight: '700', textAlign: 'right' }}>کریڈٹ (ادائیگی)</th>
                      <th style={{ padding: '1rem 1.5rem', color: '#475569', fontWeight: '700', textAlign: 'right' }}>باقی ماندہ رقم</th>
                    </tr>
                  </thead>
                  <tbody>
                    {supplierLedger.length === 0 ? (
                      <tr><td colSpan="6" style={{ padding: '5rem', textAlign: 'center', color: '#94a3b8', fontSize: '1.1rem' }}>اس سپلائر کے لیے کوئی لیجر ہسٹری نہیں ملی۔</td></tr>
                    ) : (
            ...
            <div style={{ padding: '1rem 2rem', borderTop: '1px solid #e2e8f0', background: '#f8fafc', textAlign: 'right', color: '#64748b', fontSize: '0.85rem' }}>
              اس سپلائر کے تمام لین دین دکھا رہا ہے
            </div>
          </div>
        </div>
      )}
        ...
        <div className="modal-overlay">
          <div className="supplier-modal">
            <div className="modal-header">
              <h2>{editingSupplier ? 'سپلائر میں ترمیم کریں' : 'نیا سپلائر شامل کریں'}</h2>
              <button className="btn-close" onClick={() => setIsModalOpen(false)}><X size={20} /></button>
            </div>
            <form onSubmit={handleSubmit}>
              <div className="modal-form" style={{ padding: '1.5rem', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                <div className="form-group"><label>کمپنی / سپلائر کا نام</label><input type="text" name="name" className="auth-input" style={{ paddingLeft: '1rem' }} value={form.name} onChange={handleInputChange} required /></div>
                <div className="form-group"><label>رابطہ شخص (اختیاری)</label><input type="text" name="contactPerson" className="auth-input" style={{ paddingLeft: '1rem' }} value={form.contactPerson} onChange={handleInputChange} /></div>
                <div className="form-group"><label>فون نمبر</label><input type="text" name="phone" className="auth-input" style={{ paddingLeft: '1rem' }} value={form.phone} onChange={handleInputChange} /></div>
                <div className="form-group"><label>ای میل ایڈریس</label><input type="email" name="email" className="auth-input" style={{ paddingLeft: '1rem' }} value={form.email} onChange={handleInputChange} /></div>
                <div className="form-group"><label>مکمل پتہ</label><input type="text" name="address" className="auth-input" style={{ paddingLeft: '1rem' }} value={form.address} onChange={handleInputChange} /></div>
              </div>
              <div className="modal-footer" style={{ padding: '1.5rem', borderTop: '1px solid #e2e8f0', display: 'flex', justifyContent: 'flex-end', gap: '1rem' }}>
                <button type="button" className="btn-secondary" onClick={() => setIsModalOpen(false)}>منسوخ کریں</button>
                <button type="submit" className="btn-primary">سپلائر محفوظ کریں</button>
              </div>
            </form>
          </div>
        </div>
