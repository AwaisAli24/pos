...
const UrduCustomers = () => {
  ...
  const printPaymentReceipt = (data) => {
    ...
    const html = `
      ...
      <title>ادائیگی رسید</title>
      <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        html, body { height: auto; }
        body { font-family: Arial, sans-serif; font-size: 12px; color: #000; width: 72mm; padding: 4mm; text-align: center; direction: rtl; }
        ...
        .title { font-weight: 800; font-size: 11px; margin: 4px 0; }
        .info { text-align: right; font-size: 11px; line-height: 1.8; }
        ...
        .note { margin-top: 6px; font-size: 10px; font-style: italic; text-align: right; }
        ...
        <div class="dash"></div>
        <p class="title">ادائیگی کی تصدیق</p>
        <div class="dash"></div>
        <div class="info">
          <p><b>رسید نمبر:</b> ${data.receiptId}</p>
          <p><b>تاریخ:</b> ${data.date.toLocaleString('ur-PK')}</p>
          <p><b>کیشیئر:</b> ${data.cashier}</p>
          <p><b>کسٹمر:</b> ${data.customerName}</p>
          ${data.customerPhone ? `<p><b>فون:</b> ${data.customerPhone}</p>` : ''}
        </div>
        <div class="dash"></div>
        <div class="row"><span>پچھلا بقایا</span><span><b>Rs. ${data.previousBalance.toLocaleString()}</b></span></div>
        <div class="row"><span>ادائیگی کا طریقہ</span><span>${data.paymentMethod}</span></div>
        <div class="total-row"><span>ادا کردہ رقم</span><span>Rs. ${data.amount.toLocaleString()}</span></div>
        <div class="balance-row"><span>بقایا رقم</span><span>Rs. ${data.newBalance.toLocaleString()}</span></div>
        ${data.description ? `<p class="note">نوٹ: ${data.description}</p>` : ''}
        <div class="dash"></div>
        <p class="thanks">آپ کی ادائیگی کا شکریہ!</p>
        <div class="footer">
    ...
    <div className="dashboard-container urdu-rtl" style={{ direction: 'rtl', fontFamily: 'Noto Nastaliq Urdu, sans-serif' }}>
      ...
      <main className="dashboard-main" style={{ display: 'block', overflowY: 'auto', paddingBottom: '3rem' }}>
        <header className="dashboard-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
          <div>
            <h1>کسٹمر مینجمنٹ (CRM)</h1>
            <p>مہمانوں کی وفاداری، ان کے اخراجات، اور رابطہ کی معلومات کا ریکارڈ رکھیں</p>
          </div>
          <button className="btn-primary" onClick={openAddModal} style={{ display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
            <Plus size={18}/> نیا کسٹمر
          </button>
        </header>
        ...
        <div className="sales-controls">
           <div className="search-wrapper" style={{ position: 'relative', width: '400px' }}>
             ...
             <input 
               type="text" 
               placeholder="کسٹمر کا نام یا فون نمبر تلاش کریں..." 
               value={searchTerm}
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr style={{ background: '#f8fafc', borderBottom: '1px solid #e2e8f0', textAlign: 'left' }}>
                <th style={{ padding: '1rem', color: '#64748b', fontWeight: '600', textAlign: 'right' }}>کسٹمر کا نام</th>
                <th style={{ padding: '1rem', color: '#64748b', fontWeight: '600', textAlign: 'right' }}>رابطہ نمبر</th>
                <th style={{ padding: '1rem', color: '#64748b', fontWeight: '600', textAlign: 'center' }}>کل اخراجات</th>
                <th style={{ padding: '1rem', color: '#64748b', fontWeight: '600', textAlign: 'center' }}>بقایا جات</th>
                <th style={{ padding: '1rem', textAlign: 'left', color: '#64748b', fontWeight: '600' }}>ایکشن</th>
              </tr>
            </thead>
            <tbody>
                      ...
                      <td colSpan="5" style={{ background: '#f8fafc', padding: '1.5rem' }}>
                        <div style={{ background: 'white', borderRadius: '12px', border: '1px solid #e2e8f0', padding: '1rem' }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '1rem', color: '#1e293b' }}>
                            <ShoppingBag size={18} /> <h3 style={{ fontSize: '0.95rem', fontWeight: '700' }}>{customer.name} کی خریداری کی ہسٹری</h3>
                          </div>
                          {salesLoading ? <p>ہسٹری لوڈ ہو رہی ہے...</p> : (
                            <table style={{ width: '100%', fontSize: '0.85rem', borderCollapse: 'collapse' }}>
                              <thead>
                                <tr style={{ color: '#64748b', textAlign: 'left', borderBottom: '1px solid #f1f5f9' }}>
                                  <th style={{ padding: '0.5rem' }}>تاریخ</th>
                                  <th style={{ padding: '0.5rem' }}>انوائس کی شناخت</th>
                                  <th style={{ padding: '0.5rem' }}>اشیاء</th>
                                  <th style={{ padding: '0.5rem' }}>طریقہ</th>
                                  <th style={{ padding: '0.5rem', textAlign: 'right' }}>کل رقم</th>
                                </tr>
                              </thead>
                              <tbody>
                                {customerSales.length === 0 ? (
                                  <tr><td colSpan="5" style={{ padding: '1rem', textAlign: 'center', color: '#94a3b8' }}>اس کسٹمر کے لیے کوئی ریکارڈ نہیں ملا۔</td></tr>
                                ) : (
        ...
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(15,23,42,0.6)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1100 }}>
          <div style={{ background: 'white', borderRadius: '16px', width: '90%', maxWidth: '420px', boxShadow: '0 25px 50px -12px rgba(0,0,0,0.25)', overflow: 'hidden' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '1rem 1.5rem', background: '#f0fdf4', borderBottom: '1px solid #bbf7d0' }}>
              <h2 style={{ fontSize: '1rem', fontWeight: '800', color: '#166534' }}>✅ ادائیگی کی رسید</h2>
            ...
            <div id="payment-receipt-preview" style={{ padding: '1.5rem', fontFamily: 'sans-serif', fontSize: '0.9rem', color: '#0f172a', textAlign: 'center' }}>
              ...
              <p style={{ fontWeight: '800', fontSize: '1rem', letterSpacing: '1px', margin: '6px 0' }}>ادائیگی کی رسید</p>
              <div style={{ borderBottom: '1.5px dashed #000', margin: '8px 0' }}></div>
              <div style={{ textAlign: 'left', fontSize: '0.85rem', lineHeight: '1.8' }}>
                <p><b>رسید کی شناخت:</b> {paymentReceiptData.receiptId}</p>
                <p><b>تاریخ:</b> {paymentReceiptData.date.toLocaleString()}</p>
                <p><b>کیشیئر:</b> {paymentReceiptData.cashier}</p>
                <p><b>کسٹمر:</b> {paymentReceiptData.customerName}</p>
                {paymentReceiptData.customerPhone && <p><b>فون:</b> {paymentReceiptData.customerPhone}</p>}
              </div>
              <div style={{ borderTop: '1px dashed #000', margin: '10px 0' }}></div>
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.85rem', marginBottom: '4px' }}>
                <span>پچھلا بیلنس</span>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.9rem', fontWeight: '700', background: paymentReceiptData.newBalance > 0 ? '#fee2e2' : '#dcfce7', padding: '6px 10px', borderRadius: '6px' }}>
                <span>باقی ماندہ رقم</span>
                <span style={{ color: paymentReceiptData.newBalance > 0 ? '#ef4444' : '#16a34a' }}>Rs. {paymentReceiptData.newBalance.toLocaleString()}</span>
              </div>
              {paymentReceiptData.description && <p style={{ marginTop: '10px', fontSize: '0.8rem', color: '#64748b', fontStyle: 'italic' }}>نوٹ: {paymentReceiptData.description}</p>}
              <div style={{ borderTop: '1px dashed #000', margin: '12px 0 4px 0' }}></div>
              <p style={{ fontSize: '0.75rem', color: '#94a3b8' }}>آپ کی ادائیگی کا شکریہ!</p>
            </div>
            {/* Actions */}
            <div style={{ display: 'flex', gap: '8px', padding: '1rem 1.5rem', borderTop: '1px solid #e2e8f0', background: '#f8fafc' }}>
              <button onClick={() => printPaymentReceipt(paymentReceiptData)} style={{ flex: 1, background: '#1e293b', color: 'white', border: 'none', padding: '0.75rem', borderRadius: '10px', fontWeight: '700', cursor: 'pointer', fontSize: '0.9rem' }}>🖨️ پرنٹ کریں</button>
              <button onClick={() => setPaymentReceiptData(null)} style={{ flex: 1, background: 'white', color: '#64748b', border: '1px solid #e2e8f0', padding: '0.75rem', borderRadius: '10px', fontWeight: '700', cursor: 'pointer', fontSize: '0.9rem' }}>بند کریں</button>
            </div>
          </div>
        </div>
      )}
          ...
          <div style={{ background: 'white', borderRadius: '16px', width: '90%', maxWidth: '400px', boxShadow: '0 25px 50px -12px rgba(0, 0, 0, 0.25)', overflow: 'hidden' }}>
            <form onSubmit={handlePaymentSubmit}>
              <div style={{ display: 'flex', justifyContent: 'space-between', padding: '1.2rem 1.5rem', borderBottom: '1px solid #e2e8f0', background: '#f8fafc' }}>
                <h2 style={{ fontSize: '1.1rem', fontWeight: '700' }}>ادھار ادائیگی وصول کریں</h2>
              ...
              <div style={{ padding: '1.5rem', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                <div>
                  <label style={{ fontSize: '0.85rem', fontWeight: '700', color: '#64748b', display: 'block', marginBottom: '0.4rem' }}>ادائیگی کی رقم (Rs.)</label>
                  <input 
                    type="number" 
                    required 
                    placeholder="وہ رقم درج کریں جو کسٹمر ابھی ادا کر رہا ہے..."
                ...
                <div>
                  <label style={{ fontSize: '0.85rem', fontWeight: '700', color: '#64748b', display: 'block', marginBottom: '0.4rem' }}>ادائیگی کا طریقہ</label>
                  <select 
                    value={paymentData.paymentMethod} 
                    onChange={(e) => setPaymentData({ ...paymentData, paymentMethod: e.target.value })} 
                    style={{ width: '100%', padding: '0.8rem', borderRadius: '8px', border: '1px solid #cbd5e1', fontSize: '1rem', outline: 'none' }}
                  >
                    <option value="Cash">نقد کی منتقلی</option>
                    <option value="Bank">بینک / آن لائن ٹرانسفر</option>
                    <option value="Card">ٹرمینل / کارڈ</option>
                  </select>
                </div>
                <div>
                  <label style={{ fontSize: '0.85rem', fontWeight: '700', color: '#64748b', display: 'block', marginBottom: '0.4rem' }}>اندرونی نوٹ</label>
                  <textarea 
                    placeholder="مثلاً پچھلے انوائس کے لیے ادائیگی..."
        ...
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(15, 23, 42, 0.6)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
          <div style={{ background: 'white', borderRadius: '16px', width: '95%', maxWidth: '1100px', maxHeight: '90vh', boxShadow: '0 25px 50px -12px rgba(0, 0, 0, 0.25)', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', padding: '1.5rem 2rem', borderBottom: '1px solid #e2e8f0', background: '#f8fafc' }}>
              <h2 style={{ fontSize: '1.25rem', fontWeight: '800', color: '#0f172a' }}>📊 کسٹمر مارکیٹ لیجر ہسٹری</h2>
               ...
               <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.92rem' }}>
                 <thead style={{ position: 'sticky', top: 0, zIndex: 10, background: '#f8fafc' }}>
                   <tr style={{ background: '#f8fafc', textAlign: 'left', borderBottom: '2px solid #e2e8f0' }}>
                     <th style={{ padding: '1rem 1.5rem', color: '#475569', fontWeight: '700' }}>تاریخ</th>
                     <th style={{ padding: '1rem 1.5rem', color: '#475569', fontWeight: '700' }}>قسم</th>
                     <th style={{ padding: '1rem 1.5rem', color: '#475569', fontWeight: '700' }}>تفصیل</th>
                     <th style={{ padding: '1rem 1.5rem', color: '#475569', fontWeight: '700', textAlign: 'right' }}>ڈیبٹ (+)</th>
                     <th style={{ padding: '1rem 1.5rem', color: '#475569', fontWeight: '700', textAlign: 'right' }}>کریڈٹ (-)</th>
                     <th style={{ padding: '1rem 1.5rem', color: '#475569', fontWeight: '700', textAlign: 'right' }}>موجودہ بیلنس</th>
                   </tr>
                 </thead>
                 <tbody>
                   {customerLedger.length === 0 ? (
                     <tr><td colSpan="6" style={{ padding: '5rem', textAlign: 'center', color: '#94a3b8', fontSize: '1.1rem' }}>اس اکاؤنٹ کے لیے کوئی لیجر ہسٹری نہیں ملی۔</td></tr>
                   ) : (
            ...
            <div style={{ padding: '1rem 2rem', borderTop: '1px solid #e2e8f0', background: '#f8fafc', textAlign: 'right', color: '#64748b', fontSize: '0.85rem' }}>
              منتخب مدت کے لیے تمام لین دین دکھا رہا ہے
            </div>
          </div>
        </div>
      )}
      ...
      {isFormOpen && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(15, 23, 42, 0.6)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 999 }}>
          <div style={{ background: 'white', borderRadius: '16px', width: '90%', maxWidth: '500px', boxShadow: '0 25px 50px -12px rgba(0, 0, 0, 0.25)', overflow: 'hidden' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', padding: '1.5rem 2rem', borderBottom: '1px solid #e2e8f0', background: '#f8fafc' }}>
              <h2 style={{ fontSize: '1.2rem' }}>{isEditing ? 'کسٹمر کی پروفائل میں ترمیم کریں' : 'نیا کسٹمر رجسٹر کریں'}</h2>
            ...
            <form onSubmit={handleSubmit} style={{ padding: '1.5rem 2rem', display: 'flex', flexDirection: 'column', gap: '1.2rem' }}>
               ...
               <div style={{ display: 'flex', flexDirection: 'column', gap: '0.4rem' }}>
                  <label style={{ fontWeight: '600', color: '#475569', fontSize: '0.9rem' }}>مکمل نام *</label>
               ...
               <div style={{ display: 'flex', flexDirection: 'column', gap: '0.4rem' }}>
                  <label style={{ fontWeight: '600', color: '#475569', fontSize: '0.9rem' }}>رابطہ نمبر</label>
               ...
               <div style={{ display: 'flex', flexDirection: 'column', gap: '0.4rem' }}>
                  <label style={{ fontWeight: '600', color: '#475569', fontSize: '0.9rem' }}>ای میل ایڈریس</label>
               ...
               <div style={{ display: 'flex', flexDirection: 'column', gap: '0.4rem' }}>
                  <label style={{ fontWeight: '600', color: '#475569', fontSize: '0.9rem' }}>شپنگ / گھر کا پتہ</label>
               ...
               <div style={{ display: 'flex', gap: '1rem', justifyContent: 'flex-end' }}>
                 ...
                 <button type="submit" style={{ padding: '0.8rem 1.5rem', background: '#2563eb', border: 'none', borderRadius: '8px', color: 'white', fontWeight: '600', cursor: 'pointer', boxShadow: '0 4px 10px rgba(37, 99, 235, 0.2)' }}>
                   {isEditing ? 'پروفائل محفوظ کریں' : 'ڈیٹا بیس میں شامل کریں'}
                 </button>
               </div>
            </form>
          </div>
