...
const ATTENDANCE_STATUSES = [
  { en: 'Present', ur: 'حاضر' },
  { en: 'Absent', ur: 'غیر حاضر' },
  { en: 'Late', ur: 'تاخیر' },
  { en: 'Half-day', ur: 'آدھا دن' },
  { en: 'Holiday', ur: 'چھٹی' }
];
...
const UrduHR = () => {
  ...
  const handleDelete = async (id) => {
    if (!window.confirm('کیا آپ واقعی اس ملازم کا ریکارڈ حذف کرنا چاہتے ہیں؟')) return;
    try {
    ...
    <div className="inventory-container urdu-rtl" style={{ direction: 'rtl', fontFamily: 'Noto Nastaliq Urdu, sans-serif' }}>
      ...
      <main className="inventory-main">
        ...
        <div className="inventory-header">
          <div>
            <h1 style={{ fontSize: '1.8rem', fontWeight: '800', color: 'var(--text-main)' }}>انسانی وسائل (HR)</h1>
            <p style={{ color: '#64748b', marginTop: '0.3rem' }}>ملازمین، حاضری اور تنخواہوں کا انتظام کریں</p>
          </div>
        ...
        <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '1.5rem', borderBottom: '2px solid #e2e8f0', paddingBottom: '0' }}>
          {[
            { tab: 'employees', label: '👤 ملازمین' },
            { tab: 'attendance', label: '📅 روزانہ حاضری' },
            { tab: 'report', label: '📊 حاضری رپورٹ' }
          ].map(({ tab, label }) => (
          ...
          <>
            <div style={{ display: 'flex', gap: '1rem', marginBottom: '1.5rem' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', background: 'white', border: '1px solid #e2e8f0', borderRadius: '10px', padding: '0.5rem 1rem', flex: 1 }}>
                ...
                <input type="text" placeholder="نام یا عہدے سے تلاش کریں..." value={searchTerm}
                  onChange={e => setSearchTerm(e.target.value)}
                  style={{ border: 'none', outline: 'none', fontSize: '0.9rem', width: '100%', textAlign: 'right' }} />
              </div>
            </div>

            {loading ? <p style={{ color: '#94a3b8', textAlign: 'center', padding: '2rem' }}>لوڈنگ ہو رہی ہے...</p> : (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                {filtered.length === 0 && (
                  <div style={{ textAlign: 'center', padding: '4rem', color: '#94a3b8' }}>
                    <UserCheck size={48} style={{ opacity: 0.3, marginBottom: '1rem' }} />
                    <p>کوئی ملازم نہیں ملا۔ پہلا ملازم شامل کریں!</p>
                  </div>
                )}
                {filtered.map(emp => (
                  <div key={emp._id} style={{ background: 'white', borderRadius: '16px', border: '1px solid #e2e8f0', overflow: 'hidden' }}>
                    ...
                    <div style={{ padding: '1.2rem 1.5rem', display: 'flex', alignItems: 'center', gap: '1rem' }}>
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '0.6rem', flexWrap: 'wrap' }}>
                          ...
                          <span style={{ background: emp.status === 'Active' ? '#dcfce7' : '#fef2f2', color: emp.status === 'Active' ? '#16a34a' : '#dc2626', fontSize: '0.75rem', fontWeight: '700', padding: '0.2rem 0.6rem', borderRadius: '20px' }}>
                            {emp.status === 'Active' ? 'فعال' : emp.status === 'Inactive' ? 'غیر فعال' : 'ختم شدہ'}
                          </span>
                        </div>
                        <p style={{ fontSize: '0.85rem', color: '#64748b', marginTop: '0.2rem' }}>
                          {emp.jobTitle || 'کوئی عہدہ نہیں'} {emp.joiningDate && ` · شمولیت: ${new Date(emp.joiningDate).toLocaleDateString('ur-PK')}`}
                        </p>
                        <div style={{ marginTop: '0.4rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                          {isSalaryPaid(emp._id) ? (
                            <span style={{ fontSize: '0.75rem', color: '#16a34a', background: '#dcfce7', padding: '0.1rem 0.6rem', borderRadius: '4px', fontWeight: 'bold', display: 'flex', alignItems: 'center', gap: '0.2rem' }}>
                              <CheckCircle size={12} /> ادا شدہ (موجودہ مہینہ)
                            </span>
                          ) : (
                            <span style={{ fontSize: '0.75rem', color: '#dc2626', background: '#fef2f2', padding: '0.1rem 0.6rem', borderRadius: '4px', fontWeight: 'bold', display: 'flex', alignItems: 'center', gap: '0.2rem' }}>
                              <AlertCircle size={12} /> غیر ادا شدہ (موجودہ مہینہ)
                            </span>
                          )}
                        </div>
                      </div>
                      ...
                      <div style={{ textAlign: 'left', flexShrink: 0 }}>
                        <p style={{ fontWeight: '700', color: 'var(--primary)', fontSize: '1rem' }}>Rs. {(emp.salary || 0).toLocaleString()}<span style={{ fontSize: '0.7rem', color: '#94a3b8', fontWeight: '400' }}>/ماہ</span></p>
                        {emp.dueAmount > 0 && <p style={{ fontSize: '0.8rem', color: '#ef4444' }}>بقایا: Rs. {emp.dueAmount.toLocaleString()}</p>}
                      </div>
                      {/* Actions */}
                    ...
                    {expandedId === emp._id && (
                      <div style={{ borderTop: '1px solid #f1f5f9', padding: '1.5rem', background: '#f8fafc', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '1.5rem' }}>
                        <div>
                          <p style={{ fontSize: '0.75rem', fontWeight: '700', color: '#94a3b8', marginBottom: '0.8rem', textTransform: 'uppercase', letterSpacing: '0.05em' }}>ذاتی معلومات</p>
                          <p style={{ fontSize: '0.85rem', color: '#475569', marginBottom: '0.4rem' }}><Phone size={13} style={{ marginLeft: '0.4rem', verticalAlign: 'middle' }} />{emp.phone || '—'}</p>
                          <p style={{ fontSize: '0.85rem', color: '#475569', marginBottom: '0.4rem' }}><MapPin size={13} style={{ marginLeft: '0.4rem', verticalAlign: 'middle' }} />{emp.address || '—'}</p>
                          <p style={{ fontSize: '0.85rem', color: '#475569' }}>عمر: {emp.age || '—'}</p>
                        </div>

                        <div>
                          <p style={{ fontSize: '0.75rem', fontWeight: '700', color: '#94a3b8', marginBottom: '0.8rem', textTransform: 'uppercase', letterSpacing: '0.05em' }}>تعلیمی و پیشہ ورانہ</p>
                          <p style={{ fontSize: '0.85rem', color: '#475569', marginBottom: '0.4rem' }}><GraduationCap size={13} style={{ marginLeft: '0.4rem', verticalAlign: 'middle' }} />{emp.education || '—'}</p>
                          <p style={{ fontSize: '0.85rem', color: '#475569' }}><Briefcase size={13} style={{ marginLeft: '0.4rem', verticalAlign: 'middle' }} />{emp.experience || '—'}</p>
                        </div>

                        <div>
                          <p style={{ fontSize: '0.75rem', fontWeight: '700', color: '#94a3b8', marginBottom: '0.8rem', textTransform: 'uppercase', letterSpacing: '0.05em' }}>حوالہ جات</p>
                        ...
                        <div>
                          <p style={{ fontSize: '0.75rem', fontWeight: '700', color: '#94a3b8', marginBottom: '0.8rem', textTransform: 'uppercase', letterSpacing: '0.05em' }}>مالیاتی تفصیل</p>
                          <p style={{ fontSize: '0.85rem', color: '#475569', marginBottom: '0.3rem' }}>تنخواہ: <strong>Rs. {(emp.salary || 0).toLocaleString()}/ماہ</strong></p>
                          <p style={{ fontSize: '0.85rem', color: '#475569', marginBottom: '0.3rem' }}>حالت: {isSalaryPaid(emp._id) ? <strong style={{ color: '#059669' }}>ادا شدہ ✓</strong> : <strong style={{ color: '#dc2626' }}>غیر ادا شدہ ✗</strong>}</p>
                          <p style={{ fontSize: '0.85rem', color: '#475569', marginBottom: '0.3rem' }}>ایڈوانس لیا گیا: <strong style={{ color: '#f97316' }}>Rs. {(emp.advanceTaken || 0).toLocaleString()}</strong></p>
                          <p style={{ fontSize: '0.85rem', color: '#475569', marginBottom: '0.3rem' }}>بقایا رقم: <strong style={{ color: '#ef4444' }}>Rs. {(emp.dueAmount || 0).toLocaleString()}</strong></p>
                          {emp.benefits && <p style={{ fontSize: '0.85rem', color: '#475569' }}>فوائد: {emp.benefits}</p>}
                        </div>

                        {emp.leavingDate && (
                          <div>
                            <p style={{ fontSize: '0.75rem', fontWeight: '700', color: '#94a3b8', marginBottom: '0.5rem', textTransform: 'uppercase' }}>ملازمت کی مدت</p>
                            <p style={{ fontSize: '0.85rem', color: '#475569' }}>شمولیت: {new Date(emp.joiningDate).toLocaleDateString('ur-PK')}</p>
                            <p style={{ fontSize: '0.85rem', color: '#ef4444' }}>چھوڑی: {new Date(emp.leavingDate).toLocaleDateString('ur-PK')}</p>
                          </div>
                        )}
                      </div>
                    )}
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem', flexWrap: 'wrap', gap: '1rem' }}>
              <div style={{ display: 'flex', gap: '1rem' }}>
                <div style={{ background: '#dcfce7', borderRadius: '10px', padding: '0.6rem 1.2rem' }}>
                  <span style={{ color: '#16a34a', fontWeight: '700' }}>✓ حاضر: {presentCount}</span>
                </div>
                <div style={{ background: '#fef2f2', borderRadius: '10px', padding: '0.6rem 1.2rem' }}>
                  <span style={{ color: '#dc2626', fontWeight: '700' }}>✗ غیر حاضر: {absentCount}</span>
                </div>
              </div>
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                <thead>
                  <tr style={{ background: '#f8fafc' }}>
                    ...
                    <th style={{ textAlign: 'right', padding: '1rem 1.5rem', fontSize: '0.8rem', color: '#64748b', fontWeight: '600', borderBottom: '1px solid #e2e8f0' }}>عہدہ</th>
                    <th style={{ textAlign: 'center', padding: '1rem 1.5rem', fontSize: '0.8rem', color: '#64748b', fontWeight: '600', borderBottom: '1px solid #e2e8f0' }}>حالت</th>
                  </tr>
                </thead>
                <tbody>
                        ...
                        <td style={{ padding: '1rem 1.5rem', textAlign: 'center' }}>
                          <div style={{ display: 'flex', gap: '0.4rem', justifyContent: 'center', flexWrap: 'wrap' }}>
                            {ATTENDANCE_STATUSES.map(s => (
                              <button key={s.en} onClick={() => !isCashier && markAttendance(emp._id, s.en)}
                                disabled={isCashier}
                                style={{
                                  padding: '0.3rem 0.8rem', border: 'none', borderRadius: '20px', fontSize: '0.78rem', fontWeight: '600',
                                  cursor: isCashier ? 'default' : 'pointer',
                                  background: status === s.en ? STATUS_COLORS[s.en].bg : '#f1f5f9',
                                  color: status === s.en ? STATUS_COLORS[s.en].text : '#94a3b8',
                                  outline: status === s.en ? `2px solid ${STATUS_COLORS[s.en].text}` : 'none'
                                }}>
                                {s.ur}
                              </button>
                            ))}
                  ...
                  {employees.filter(e => e.status === 'Active').length === 0 && (
                    <tr><td colSpan="3" style={{ textAlign: 'center', padding: '3rem', color: '#94a3b8' }}>کوئی فعال ملازم نہیں ملا۔</td></tr>
                  )}
                </tbody>
              </table>
            </div>
          ...
          <>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem', flexWrap: 'wrap', gap: '1rem' }}>
              <div style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap' }}>
                <div style={{ background: '#dcfce7', borderRadius: '10px', padding: '0.6rem 1.2rem' }}>
                  <span style={{ color: '#16a34a', fontWeight: '700' }}>✓ کل حاضر: {attendanceSummary.reduce((s, e) => s + (e.Present || 0), 0)}</span>
                </div>
                <div style={{ background: '#fef2f2', borderRadius: '10px', padding: '0.6rem 1.2rem' }}>
                  <span style={{ color: '#dc2626', fontWeight: '700' }}>✗ کل غیر حاضر: {attendanceSummary.reduce((s, e) => s + (e.Absent || 0), 0)}</span>
                </div>
                <div style={{ background: '#fef9c3', borderRadius: '10px', padding: '0.6rem 1.2rem' }}>
                  <span style={{ color: '#ca8a04', fontWeight: '700' }}>⏰ کل تاخیر: {attendanceSummary.reduce((s, e) => s + (e.Late || 0), 0)}</span>
                </div>
              </div>
            ...
            <div style={{ background: 'white', borderRadius: '16px', border: '1px solid #e2e8f0', overflow: 'hidden' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                <thead>
                  <tr style={{ background: '#f8fafc' }}>
                    <th style={{ textAlign: 'right', padding: '1rem 1.5rem', fontSize: '0.8rem', color: '#64748b', fontWeight: '700', borderBottom: '1px solid #e2e8f0' }}>ملازم</th>
                    <th style={{ textAlign: 'center', padding: '1rem 1.5rem', fontSize: '0.8rem', color: '#64748b', fontWeight: '700', borderBottom: '1px solid #e2e8f0' }}>کل دن</th>
                    <th style={{ textAlign: 'center', padding: '1rem 1.5rem', fontSize: '0.8rem', color: '#16a34a', fontWeight: '700', borderBottom: '1px solid #e2e8f0' }}>✓ حاضر</th>
                    <th style={{ textAlign: 'center', padding: '1rem 1.5rem', fontSize: '0.8rem', color: '#dc2626', fontWeight: '700', borderBottom: '1px solid #e2e8f0' }}>✗ غیر حاضر</th>
                    <th style={{ textAlign: 'center', padding: '1rem 1.5rem', fontSize: '0.8rem', color: '#ca8a04', fontWeight: '700', borderBottom: '1px solid #e2e8f0' }}>⏰ تاخیر</th>
                    <th style={{ textAlign: 'center', padding: '1rem 1.5rem', fontSize: '0.8rem', color: '#2563eb', fontWeight: '700', borderBottom: '1px solid #e2e8f0' }}>½ آدھا دن</th>
                    <th style={{ textAlign: 'center', padding: '1rem 1.5rem', fontSize: '0.8rem', color: '#6b7280', fontWeight: '700', borderBottom: '1px solid #e2e8f0' }}>حاضری %</th>
                  </tr>
                </thead>
                <tbody>
                  {attendanceSummary.length === 0 && (
                    <tr><td colSpan="7" style={{ textAlign: 'center', padding: '3rem', color: '#94a3b8' }}>اس مہینے کا کوئی ریکارڈ نہیں ملا۔</td></tr>
                  )}
        ...
        <div className="modal-overlay" style={{ overflowY: 'auto', alignItems: 'flex-start', padding: '2rem 0' }}>
          <div className="product-modal" style={{ maxWidth: '700px', margin: 'auto' }}>
            <div className="modal-header">
              <h2>{editingEmp ? 'ملازم کی معلومات تبدیل کریں' : 'نیا ملازم شامل کریں'}</h2>
            ...
            <form onSubmit={handleSubmit} style={{ padding: '1.5rem', display: 'flex', flexDirection: 'column', gap: '1.2rem' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '1.5rem' }}>
                ...
                <div>
                  <label style={{ display: 'block', fontWeight: '600', marginBottom: '0.4rem', color: 'var(--text-main)' }}>ملازم کی تصویر</label>
                  <input type="file" accept="image/*" onChange={handlePhotoChange} style={{ fontSize: '0.85rem' }} />
                </div>
              </div>

              {/* Personal Info */}
              <p style={{ fontWeight: '700', color: '#475569', borderBottom: '1px solid #e2e8f0', paddingBottom: '0.5rem' }}>ذاتی معلومات</p>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
                <div className="form-group" style={{ gridColumn: '1 / -1' }}>
                  <label>مکمل نام *</label>
                  <input type="text" className="auth-input" style={{ paddingLeft: '1rem' }} value={form.fullName} onChange={e => setForm({ ...form, fullName: e.target.value })} required placeholder="مثلاً محمد علی" />
                </div>
                ...
                <div className="form-group" style={{ gridColumn: '1 / -1' }}>
                  <label>پتہ</label>
                  <input type="text" className="auth-input" style={{ paddingLeft: '1rem' }} value={form.address} onChange={e => setForm({ ...form, address: e.target.value })} placeholder="گھر کا مکمل پتہ" />
                </div>
              </div>

              {/* Job Info */}
              <p style={{ fontWeight: '700', color: '#475569', borderBottom: '1px solid #e2e8f0', paddingBottom: '0.5rem' }}>ملازمت کی معلومات</p>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
                <div className="form-group">
                  <label>عہدہ</label>
                  <input type="text" className="auth-input" style={{ paddingLeft: '1rem' }} value={form.jobTitle} onChange={e => setForm({ ...form, jobTitle: e.target.value })} placeholder="کیشیئر، منیجر وغیرہ" />
                </div>
                <div className="form-group">
                  <label>حالت</label>
                  <select className="auth-input" style={{ paddingLeft: '1rem', appearance: 'auto' }} value={form.status} onChange={e => setForm({ ...form, status: e.target.value })}>
                    ...
                    <option value="Terminated">ختم شدہ (Terminated)</option>
                  </select>
                </div>
                <div className="form-group">
                ...
                <div className="form-group" style={{ gridColumn: '1 / -1' }}>
                  <label>تعلیم</label>
                  <input type="text" className="auth-input" style={{ paddingLeft: '1rem' }} value={form.education} onChange={e => setForm({ ...form, education: e.target.value })} placeholder="میٹرک / ایف اے / بی اے وغیرہ" />
                </div>
                <div className="form-group" style={{ gridColumn: '1 / -1' }}>
                  <label>تجربہ</label>
                  <input type="text" className="auth-input" style={{ paddingLeft: '1rem' }} value={form.experience} onChange={e => setForm({ ...form, experience: e.target.value })} placeholder="2 سالہ تجربہ وغیرہ" />
                </div>
              </div>

              {/* References */}
              <p style={{ fontWeight: '700', color: '#475569', borderBottom: '1px solid #e2e8f0', paddingBottom: '0.5rem' }}>حوالہ جات</p>
              {[0, 1].map(i => (
                <div key={i} style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '0.8rem', background: '#f8fafc', padding: '1rem', borderRadius: '10px' }}>
                  <p style={{ gridColumn: '1/-1', fontSize: '0.8rem', fontWeight: '600', color: '#64748b', margin: 0 }}>حوالہ {i + 1}</p>
                  <div className="form-group">
                    <label>نام</label>
                    <input type="text" className="auth-input" style={{ paddingLeft: '1rem' }} value={form.references[i]?.name || ''} onChange={e => updateRef(i, 'name', e.target.value)} placeholder="نام" />
                  </div>
                  <div className="form-group">
                    <label>فون</label>
                    <input type="text" className="auth-input" style={{ paddingLeft: '1rem' }} value={form.references[i]?.phone || ''} onChange={e => updateRef(i, 'phone', e.target.value)} placeholder="فون" />
                  </div>
                  <div className="form-group">
                    <label>رشتہ</label>
                    <input type="text" className="auth-input" style={{ paddingLeft: '1rem' }} value={form.references[i]?.relation || ''} onChange={e => updateRef(i, 'relation', e.target.value)} placeholder="مثلاً انکل / دوست" />
                  </div>
                </div>
              ))}

              {/* Financials */}
              <p style={{ fontWeight: '700', color: '#475569', borderBottom: '1px solid #e2e8f0', paddingBottom: '0.5rem' }}>مالیاتی تفصیل</p>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
                <div className="form-group">
                  <label>ماہانہ تنخواہ (روپے)</label>
                ...
                <div className="form-group">
                  <label>ایڈوانس لیا گیا (روپے)</label>
                ...
                <div className="form-group">
                  <label>دیگر فوائد</label>
                  <input type="text" className="auth-input" style={{ paddingLeft: '1rem' }} value={form.benefits} onChange={e => setForm({ ...form, benefits: e.target.value })} placeholder="ٹرانسپورٹ، کھانا وغیرہ" />
                </div>
              </div>

              <button type="submit" className="btn-primary" style={{ marginTop: '0.5rem', fontSize: '1rem' }}>
                {editingEmp ? 'تبدیلی محفوظ کریں' : 'ملازم شامل کریں'}
              </button>
            </form>
          </div>
        </div>
