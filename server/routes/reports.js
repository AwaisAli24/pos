...
router.get('/export', auth, async (req, res) => {
  try {
    ...
    doc.addPage();
    doc.fontSize(16).text('Top Selling Products Performance', { align: 'center' }).moveDown();
    
    // Header for product table to align manually with spacing for PDF readability
    doc.fontSize(12).text(`Item Name`, 50, 100);
    doc.text(`Units Sold`, 350, 100, { align: 'center' });
    doc.text(`Total Revenue`, 450, 100, { align: 'right' });
    doc.moveDown(0.5);
    doc.moveTo(50, 115).lineTo(550, 115).stroke();

    let yPos = 130;
    topProductsInfo.forEach(p => {
      doc.fontSize(10).text(p._id, 50, yPos);
